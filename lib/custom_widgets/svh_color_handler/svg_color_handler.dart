...
import 'package:nocab/extensions/color_to_hex.dart';

class SvgColorHandler extends StatelessWidget {
  final String svgPath;
  final Map<Color, Color> colorSwitch;
  final double height;
  const SvgColorHandler({Key? key, required this.svgPath, required this.colorSwitch, this.height = 150}) : super(key: key);
        ...
        builder: (context, snapshot) {
          return SvgPicture.string(
            colorSwitch.entries.fold(
              ...
              (String svg, e) => svg.replaceAll(RegExp(e.key.toHex(), caseSensitive: false), e.value.toHex()),
            ),
            height: height,
          );
        });
